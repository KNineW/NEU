...
class ContentLossLayer(object):
    ...
    def forward(self, input_layer, content_layer):
        ...
        loss = np.sum(0.5*np.square(input_layer-content_layer))/input_layer.size
        return loss
...
class StyleLossLayer(object):
    ...
    def forward(self, input_layer, style_layer):
        ...
        self.gram_style =  np.expand_dims(np.dot(style_layer_reshape[0], style_layer_reshape[0].T), axis=0)
        ...
        style_diff = 0.25 * np.square(self.gram_input - self.gram_style) / self.div
        loss = np.sum(style_diff)/input_layer.shape[0]
        return loss
    def backward(self, input_layer, style_layer):
        ...
        for idxn in range(input_layer.shape[0]):
            # TODO： 计算风格损失的反向传播
            bottom_diff[idxn, :, :] = np.dot(self.input_layer_reshape[idxn].T, self.gram_input[idxn] - self.gram_style[idxn]).T / self.div
